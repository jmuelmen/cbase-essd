...
bases.cbase <- function(path = "/projekt3/climate/DATA/SATELLITE/MULTI_SENSOR/DARDAR/DARDAR_MASK/2007",
    ...
    ## %>%
    ##     ## ungroup() %>%
    ##     ## ddply(~ time, function(x) {
    ##     ##     with(x)
    ##     as.data.frame() %>%
    ##     select(-time) %>%
    ##     filter(feature.above.surface == "clear air") %>%
    ##     head(50)
    
    ## str()
    
    
    ## label.lowest.cloud <- max(labels(f2[,1] == "cloud"))
    ## feature.qa.lowest.cloud <- min(Feature_Type_QA[labels == ])
    ## phase.lowest.cloud <- 
    ## stack <- vertical.features.stack(f2[,1])
    ## stack.idx.surface <- which(stack == "surface")

    
    ## labels <- label.vertical.features(Feature_Type[,1])
    ## label.sfc <- labels[f2[,1] == "surface"]
    ## label.lowest.cloud <- max(labels(f2[,1] == "cloud"))
    ## feature.qa.lowest.cloud <- min(Feature_Type_QA[labels == ])
    ## phase.lowest.cloud <- 
    ## stack <- vertical.features.stack(f2[,1])
    ## stack.idx.surface <- which(stack == "surface")
    ## if (idx.surface > 1)
    ##     feature.above.surface <- stack[stack.idx.surface - 1]
