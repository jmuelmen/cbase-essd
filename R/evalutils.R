...
regression_plot <- function(df, title, xlab = "50 km running mean CALIOP base (km)") {
    stats <- statistify(df)
    gg <- ggplot(df, aes(caliop, ceilo)) +
        geom_point(pch = ".") + stat_density2d(geom = "density2d") +
        stat_smooth(method = "lm", col = "red", fill = "red", formula = y ~ x) +
        stat_smooth(method = "loess", col = "blue", fill = "blue", formula = y ~ x) +
        geom_abline(intercept = 0, slope = 1000, lty = "dashed", col = "lightgrey") +
        coord_cartesian(ylim = c(-500,5500)) +
        facet_grid(groups(df)) +
        labs(title = title, x = xlab, y = "Ceilometer base (m)") +
        geom_text(data = stats, aes(x = 5, y = 0,
