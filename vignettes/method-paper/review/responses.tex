...
We concede that the SVM-based correction is a black box.  We have attempted to
explain the correction by analogy to a linear correction.  Quantification is
nontrivial because the correction is a function of $z$, $D$, $n$, and
$\Delta z$.  \commentjm{Maybe show the ratio of uncorrected to corrected?}  The
correction is desirable because otherwise (a) the average over many CBASE
...
summarize succinctly.  \commentjm{But we should give some highlights, like what
  is the best and worst category.}  However, in the R package that will be made
public to accompany the final manuscript, the full predicted uncertainty table
for each of the 375 categories in $D$, $n$, and $\Delta z$ is available.
