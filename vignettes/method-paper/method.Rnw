...
complicating objective comparison to satellite CBH.  \comment{JQ}{It would be
  good to provide a table or map of the stations, and write a little more on how
  many there are, and what the temporal resolution is.}
...
native temporal resolution, co-located to the CALIPSO  \commentjm{expand?} overpass (see below), at
those points.}  A statistically independent dataset is required for an unbiased
