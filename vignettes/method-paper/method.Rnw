...
\affil[1]{Institute for Meteorology, Universit\"at Leipzig, Leipzig, Germany}
\affil[2]{Institute for Geophysics and Meteorology, Universit\"at zu K\"oln, K\"oln, Germany}
...
\begin{abstract}
  A retrieval technique is presented that uses attenuated backscatter profiles
  ...
  by less than 10\%. {\bf perhaps a few words on some results, such as
  global-average cloud base height, or a little, more land/ocean or so?}
\end{abstract}

\introduction  %% \introduction[modified heading if necessary]
\label{sec:intro}
Cloud base height is an important geometric parameter of a cloud.  For example,
it controls how much downwelling longwave radiation the cloud emits; aerosol
concentration and updraft speed at that level determine the microphysical characteristics of the
cloud; it is one of the parameters that is required in the calculation of the
subadiabaticity of the cloud.  However, due to the viewing geometry, it is also
one of the most difficult cloud parameters to retrieve from satellite.

Multiple methods have been proposed for satellite determination of the cloud
base height.  The perhaps simplest one makes use of the retrieved cloud-top
height and cloud liquid water path. Inferring a cloud-top height from
ancillary (re-analysis) temperature profile data, and assuming an adiabatic
vertical profile of specific cloud liquid water, cloud base height can be inferred (Zhu et al.~2014, doi
10.1002/2013GL058970). Both, the use of ancillary temperature profile
estimates, and the assumption of an adiabatic cloud profile, may be
problematic in many cases.\par
Results of evaluating cloud base heights inferred from
oxygen absorption bands {\bf but still it would be good to sketch the idea
here and provide a reference?} and from MISR stereoscopic images will be reported in
separate manuscripts.  For analyses wishing to combine cloud base information
with other cloud properties retrieved by A-Train satellites, the latter two methods share
...
ceilometers.\par
The cloud base we are interested in is the base height of the lowest cloud in
each column. Even if it is possible for CALIOP to also detect the base height
of multi-layer clouds in certain situations, it is for many applications the
base height of the lowest cloud that is of largest interest. Also, the central
assumption of horizontal homogeneity at mesoscales of the cloud base height is
valid only for clouds that are rooted in the planetary boundary layer.
...
Two classes of data are used in this work.  The first class is data from the
satellite cloud lidar,
which we use with the intent of deriving a global data set of cloud base height.
The second class is ground-based observations as reference measurements of cloud base height used
...
(``horizontal averaging distance'').  The VFM in particular reports levels in
which the lidar completely attenuates, and it also reports whether the land-
or ocean surface delivered a return signal to the satellite. In the present
study, we use these two pieces of information in combination with cloudy
levels (all cloud groups lumped together) and non-cloudy pixels (all other
detectable features - including ``clear air'' and ``aerosols'' - lumped together).
...
of the tuned algorithm, reference measurements of cloud base heights are required.  The source
...
field as a proxy for spatial variability of the cloud field \citep[e.g.,]{heese2010,doi:10.5194/amt-3-1763-2010}.
...
{\bf It would be good to provide a table or map of the stations, and write a
little more on how many there are , and what the temporal resolution is}
...
\subsection{Colocation of satellite- and groundbased observations}
{\bf A section on the criteria selected for co-locating ceilometer and CALIPSO
shots?}
...
\item We determine the cloud base height from all CALIOP profiles where the
  surface generates a return. This is the only case in which we can be sure
  that CALIOP reliably detected the base of the lowest cloud in the column,
  i.e. that the lidar is not attenuated above the base of the lowest
  cloud.  We refer to these cloud base heights as \textit{local cloud base
...
(ceilometer-observed) cloud base height.  We use ceilometer observations from
YYYY stations over the continental USA from
the year 2008 at native temporal resolution, co-located to the CALIPSO
overpass (see below), at those points.  A statistically independent data set is required
...
\end{itemize}
{\bf we should write here how these qualitative assessments translate into
...
obscures {\bf but we excluded that possibility, didn't we?} a lower layer.  \commentjm{Isn't the actual reason a combination of
...
described in Section~\ref{sec:algorithm:qual}. {\bf could we create a sketch
explaining this procedure?}

For each  local cloud base height $z_j$ within the circle with radius
$D_\text{max}$ around the POI, we determine an 
uncertainty $\sigma_j$ based on the categories established in the previous
...
{\bf I would prefer shifting this correction (the following paragraph) to the local CALIOP base heights
...
$(z - \hat{z})/\sigma$ has to a very good approximation zero mean and unit standard deviation. In our
...
\begin{figure*}
  ...
  \caption{Scatter plots of CALIOP versus ceilometer local cloud base height
    ...
    Table~\ref{tab:quality-qa}. {\bf I would prefer square-shaped plots, and a
...
\begin{table*}
  ...
  \caption{Statistics of the relationship between ceilometer and CALIOP cloud
    ...
    fit parameters. {\bf better write 1050 instead of $1.05\times10^3$, same
  for the offset in the fit}}
...
\begin{figure*}
  ...
  \caption{Scatter plot of CBASE versus ceilometer cloud base height; for
    ...
    $\Sexpr{res$stats$icpt}$~m. {\bf why not have the origin at (0,0)? What are
...
\begin{figure*}
  ...
  \caption{Scatter plot of 2B-GEOPROF-LIDAR versus ceilometer cloud base height
    ...
    \commentjm{I \textit{think} this uses Claudia's algorithm (minimum cloud
      base within $D_\text{max}$), but I have to check.} {\bf also square
  plots, if possible?}}
  \label{fig:eval-2b}
\end{figure*}
