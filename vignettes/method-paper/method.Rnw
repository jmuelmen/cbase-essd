...
base height.  \commentjm{VIIRS \commentjm{expand?} cloud-base temperature method: \cite{zhu14}}.  Results of
...
combination of CloudSat and CALIOP \citep[2B-GEOPROF-LIDAR][]{mace14}.  Each of
...
intent to derive a global CBH dataset; and ground-based observations used as
...
measurements of CBH are required.  The source of these ``true'' CBH is
...
henceforth][]{metar}.  METAR data is used, among other purposes, for
assimilation in NWP \citep[e.g.,][]{HRRR, IFS}.  In many locations, CBH reported
in METARs are measured by ceilometer over a period of time (tens of minutes) and
then objectively grouped into cloud layers and their respective fractional
coverages, using the temporal variation at a fixed point under an advected cloud
field as a proxy for the spatial variability of the cloud field
\citep[e.g.,]{heese2010,doi:10.5194/amt-3-1763-2010}.  METAR data is distributed
...
than 5000~feet, complicating objective comparison to satellite CBH.
\comment{JQ}{It would be good to provide a table or map of the stations, and
  write a little more on how many there are, and what the temporal resolution
  is.}
