...
10.1002/2013GL058970).  Results of evaluating CBH inferred from
...
henceforth]{metar}.  In many locations, CBH reported in METARs are measured by
ceilometer over a period of time (tens of minutes) and then objectively grouped
into cloud layers and their respective fractional coverages, using the temporal
variation at a fixed point under an advected cloud field as a proxy for spatial
...
We assess the quality of the CALIOP CBH $z$ using the
...
select all local CALIOP CBH within a horizontal distance
...
set, the ``true'' (i.e., measured by the ceilometer) CBH $\hat{z}$
...
estimate, which allows an analysis using the CBH to select only
