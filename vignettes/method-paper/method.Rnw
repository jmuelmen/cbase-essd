...
\affil[3]{Institute of Meteorology, Universit\"at K\"oln, K\"oln, Germany}
...
to train and evaluate the algorithm by which cloud base height is determined from
the satellite data.
...
cloud base uncertainty, and for evaluation of the trained algorithm,``true''
cloud base heights need to be known.  The source of these ``true'' cloud base
heights in this work are ground-based cloud observations in aviation routine and
special weather reports \citep[][METARs and SPECIs, collectively referred to as
METARs henceforth]{metar}.  In many locations, cloud base heights reported in
METARs are measured by ceilometer over a period of time (tens of minutes) and
...
the lowest cloud base is higher than 5000~feet.  The ASOS data is used, among
other purposes, for assimilation in NWP (HRRR and IFS citation).  In METAR
format, it is distributed globally by the WMO and widely archived.  The data for
the present analysis was downloaded from the Weather Underground archive
\citep{wunderground}.

To obtain an unbiased estimate of the algorithm performance, it is necessary to
use an evaluation data set that is statistically independent of the training data
set.  We use METARs from the year 2008 for training.  Once the training is complete,
...
\item Using ground-based ceilometer data, we determine quality of cloud base
  height depending on a number of factors.  \commentjm{Refer to this as
    ``training'', and define it in the context of supervised machine learning.}
