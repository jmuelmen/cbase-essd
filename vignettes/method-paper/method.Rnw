...
ceilometer-observed $\hat{z}_b$.  The RMSE is defined as
\begin{equation}
  \label{eq:rmse}
...
dataset, the ``true'' (i.e., ceilometer-measured) $\hat{z}_b$ is known in
...
uncertainties $\sigma$, the pdf of the quantity $(\CBH - \hat{z}_b)/\sigma$ has zero
...
2B-GEOPROF-LIDAR lidar-only \CBH{}, which is based on the same
...
\begin{figure*}
  ...
  \Sexpr{res$stats$slope * 1e-3} and intercept $\Sexpr{res$stats$icpt}$~m.}
  \label{fig:eval}
\end{figure*}
...
\begin{figure*}
  \centering
  <<cbase-uncert-quantiles,dev='tikz',fig.width=7,fig.height=9,out.width='0.95\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='hide'>>=
...
\begin{figure}
  \centering
  (a)\\
  \includegraphics[width=0.5\linewidth,keepaspectratio=true,angle=90]{DownwellingLW_uncert_constant400mcloudbase.eps}\\
  (b)\\
  \includegraphics[width=0.5\linewidth,keepaspectratio=true,angle=90]{DownwellingLW_uncert_derivedcloudbase.eps}
