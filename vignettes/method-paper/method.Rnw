...
estimates). {\bf I don't think the following is true, since also for frontal
clouds the base is well-defined}: Also, the central assumption of
...
footprint{\bf for the repetitive ground-track of the CALIPSO orbit (?)}.  In normal operation, the time resolution of CBH reports is 1~h, but
...
accessible in a single column and may have the substantial effects on quality are:
\begin{itemize}
...
\item {\bf this condition sounds odd given that in general we assume that CBH
is homogeneous. Is it necessary?: }minimum CALIOP cloud detection horizontal averaging distance within the
...
\item thermodynamic phase of the lowest layer determined to be other than liquid
  ...
  below) {\bf könnte man die Bedingung denn auch weglassen?}.
\end{itemize}
...
\item number of CALIOP columns $n$ with a cloud layer and a surface return
  ...
  175, 250, 325, 400 (multiplicities greater than 400 are accepted {\bf oder
  stattdessen "325, 400, $\infty$"?}),
...
\begin{figure*}
  ...
  @
  \caption{Scatter plot of CBASE versus ceilometer CBH {\bf for all data over
  the CONUS available for 2007 (?)}; for
    ...
    $\Sexpr{res$stats$icpt}$~m {\bf kann man den auf 1\,m runden?}. {\bf was
  ist mit Dunkelblau in der Farbskala, und auch hier: warum nicht beide Achsen
  in m bzw. km?}}
  \label{fig:eval}
\end{figure*}
